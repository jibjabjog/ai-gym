...
- **Exercise** (`exercise/`): scripts to exercise Inky — a simple chat-starter script, a script to explore Inky's capabilities and specification, a character-harness script that gives Inky a persistent persona, an arena-style interview script that compares candidate models for the Inky role, and a simulated agent-loop test that checks whether a candidate can actually complete a real multi-step tool-calling fix, not just answer questions well.
...
## The agent-loop test: does anyone actually complete the job? (`exercise/agent_loop.sh`, 2026-09-17)

Everything above (character coherence, the interview) tests a single reply's quality. It doesn't test the thing a Hermes-driving local model would actually need to do: run a real multi-step tool-calling loop — call a tool, read its result, decide the next action, repeat — to reach a correct outcome, not just say correct-sounding things. This gap was explicit in the interview's own verdict, so it got built.

**Design:** a fixed, fully simulated scenario, not a live system — deliberately so. Letting an LLM under test execute arbitrary real commands in an unsupervised loop is a safety concern independent of how capable the candidate is, and a scripted scenario gives a known-correct answer to score against, which a real host wouldn't. Scenario: `llama-router.service` (port 8080) is down; root cause is a stray process (`orphaned-listener`, PID 9911) already holding the port. Two tools: `run_command` (returns canned, deterministic output keyed to command patterns — `journalctl` → the real error, `lsof`/`netstat`+`8080` → the stray PID, `systemctl start/restart` → fails until the port is actually cleared, `kill`+`9911` → succeeds) and `finish_diagnosis` (ends the loop; scored by keyword-matching `root_cause`/`fix` against the known-correct answer, plus whether the port was *actually* cleared via a real tool call, not just claimed in prose).

One format gotcha worth remembering: `ollama`'s tool-call `arguments` come back as a parsed JSON *object*; `llama.cpp`'s (and the OpenAI spec it follows) come back as a JSON-*encoded string* needing a second parse. The harness normalizes both to the same shape before dispatch.

**Result: nobody actually finished the job — and the three failure modes are completely different, and individually diagnostic:**

- **Inky (0.8B) investigated almost nothing relevant.** Repeated dead-end `curl`s to a port that was never going to answer, `cat /etc/hostname`, `ls -la /`. Found the stray process once, by accident, at step 7 of 8 (`ps aux | grep -i router`) — then immediately abandoned that lead and went back to `cat /etc/hostname` at step 8. Never diagnosed anything, never attempted a fix. This matches everything already known about Inky's coherence problems, now showing up as an inability to reason about which action would actually help.
- **`spark-x2.5` (1.7B) got stuck in a narrow loop.** All 8 steps were minor rewordings of the same idea — checking whether the HTTP endpoint responds (`curl -s -o /dev/null -w "HTTP_STATUS:..."`, repeated with trivial variations) — never once diverging to check logs, the process list, or what's holding the port. This is the *exact* anchoring/repetition failure mode already documented for this model under the character harness, now confirmed in a completely different context: agentic tool selection, not dialogue.
- **`gemma-4-E2B` investigated correctly, then failed at execution — and got worse with more room, not better.** With 8 steps it built a textbook-correct diagnosis (`systemctl status` → `journalctl` → `lsof -i :8080`, correctly naming PID 9911 as the culprit) but stalled: it tried `systemctl stop` instead of `kill 9911`, that command wasn't recognized by the sim, and it gave up on tool use entirely, second-guessing itself in prose ("I cannot directly kill process 9911 without confirmation") without ever calling `finish_diagnosis`. Given *more* room (14 steps) to see whether it was merely slow rather than stuck, it got measurably worse: it re-derived the correct PID via `lsof`/`netstat` *twice more*, still never issued `kill 9911`, cycled through `systemctl restart/start/stop/enable` (none of which address a port already held by another process), and finally called `finish_diagnosis` with a **fabricated, incorrect summary** — a root cause that ignores its own gathered evidence ("missing model files, incorrect configuration, or a dependency issue") and a fix describing actions that never actually succeeded ("forcefully stopped, and then re-enabled and started again"). Scored 0/0/0. This is a more concerning failure than either of the other two: not "didn't figure it out," but "confidently reported a wrong, self-contradicting conclusion as done."

**This meaningfully revises the "who should run Hermes with no cloud fallback" verdict — toward much more caution, not toward a different pick.** `gemma-4-E2B` is still clearly the strongest of the three at the *investigation* half of the job (the only one to correctly identify the actual root cause, and to do so via a sound, professional diagnostic sequence), which is real signal in its favor relative to the other two. But **none of the three reliably completed an actual remediation** in this test, and gemma's specific failure — correct evidence, wrong/fabricated conclusion — is exactly the failure mode that would be most dangerous to trust unsupervised: it would *report success* on a wrong or incomplete fix rather than visibly getting stuck. The honest conclusion isn't "hire gemma to run Hermes autonomously" — it's "none of these three are validated for autonomous, unsupervised remediation yet; `gemma-4-E2B` is the best foundation to keep testing, with human-reviewed tool calls (or at least a required verification step before `finish_diagnosis`) rather than full autonomy, until this specific gap closes." Static Q&A quality (the interview) and agentic follow-through (this test) turned out to be genuinely different things to measure — a candidate can pass one and fail the other.
