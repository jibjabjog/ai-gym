...
- End-to-end against `spark-x2.5`: a moderate conversation (no strong kind/rude words) correctly keeps mood — and therefore the shown band — unchanged throughout, as it should. A conversation loaded with insults then heavy praise moved the mood value as expected but also **stress-tested the guardrail harder**: with a longer brief (mood section + more voice examples) on top of an already-weak 1.7B model, more turns collapsed to the fallback line than under the flat-voice-examples version. This reads as the model's limited capacity being further strained by a bigger brief, not a plumbing bug — the-orb's own devlog describes very similar model-capacity trade-offs when they added detail to their brief. Worth knowing before assuming "bigger brief = strictly better": there's a real cost to brief size against a model this small.
